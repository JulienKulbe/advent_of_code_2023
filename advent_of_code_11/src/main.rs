...
fn main() -> Result<()> {
    ...
    for galaxy in &mut galaxies {
        galaxy.y += rows.iter().filter(|&&r| r < galaxy.y).count();
        galaxy.x += columns.iter().filter(|&&c| c < galaxy.x).count();
    }
    ...
    for (index, galaxy_1) in galaxies.iter().enumerate() {
        for galaxy_2 in galaxies.iter().skip(index) {
            let d = galaxy_1.x.abs_diff(galaxy_2.x) + galaxy_1.y.abs_diff(galaxy_2.y);
            distance += d;
        }
    }

    println!("Task 1: {distance}");
